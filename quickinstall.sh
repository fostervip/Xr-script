...
function addcloudreve_configure_nginx() {
  nginx_conf="/etc/nginx/conf.d/${domain}.conf"
  cd /etc/nginx/conf.d/
  sed -i '/server {/,/}/ {
    /location \// {
...
function configure_webtest() {
  rm -rf /www/xray_web
  mkdir -p /www/xray_web
  print_ok "是否配置 Cloudreve 伪装网站？[Y/N]"
  read -r webpage
  case $webpage in
  [yY][eE][sS] | [yY])
    # 获取系统架构
    ...
    case $ARCH in
      x86_64)
        DOWNLOAD_URL="https://github.com/cloudreve/Cloudreve/releases/download/$(curl -s https://api.github.com/repos/cloudreve/Cloudreve/releases/latest | grep tag_name | cut -d '"' -f 4)/cloudreve_$(curl -s https://api.github.com/repos/cloudreve/Cloudreve/releases/latest | grep tag_name | cut -d '"' -f 4)_linux_amd64.tar.gz"
        ;;
      aarch64|arm64)
        DOWNLOAD_URL="https://github.com/cloudreve/Cloudreve/releases/download/$(curl -s https://api.github.com/repos/cloudreve/Cloudreve/releases/latest | grep tag_name | cut -d '"' -f 4)/cloudreve_$(curl -s https://api.github.com/repos/cloudreve/Cloudreve/releases/latest | grep tag_name | cut -d '"' -f 4)_linux_arm64.tar.gz"
        ;;
      *)
    ...
    if [ $? -ne 0 ]; then
      print_error "Cloudreve 下载失败"
      return
    fi
    tar xzf cloudreve.tar.gz -C /www/xray_web
    rm -f cloudreve.tar.gz

    # 移动可执行文件并设置权限
    mv /www/xray_web/cloudreve /usr/bin/cloudreve
    chmod +x /usr/bin/cloudreve

    # 创建必要目录（可选，首次运行会自动创建）
    mkdir -p /www/xray_web/{uploads,avatar,data}

    # 创建 systemd 服务（后台运行 Cloudreve）
...
EOF
    ...
    judge "Cloudreve 伪装网站安装"
    print_ok "Cloudreve 已安装并启动，首次访问你的域名:5212 端口进行初始化管理员账号"
    print_ok "注意：实际伪装中，Nginx 会反向代理到 Cloudreve 的 5212 端口"
    ;;
  *) ;;
  esac
}
